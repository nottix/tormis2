...
public class QMatrixGenerator extends Thread {
	...
	public Matrix calcQMatrix() {
		...
		for(j=this.startRow; j<states.size() && j<this.endRow; j++) {               // j - righe
			if((j%10)==0)
				System.out.println(this.getName()+" -> Status: "+j+" of "+this.endRow+", "+(((double)(j-this.startRow)/(double)(this.endRow-this.startRow))*100)+"%");
			for(i=this.startCol; i<states.size() && j<this.endCol; i++) {       // i - colon
...
//				Runtime.getRuntime().gc();
	...
	private boolean checkRsRdCondition1(BbsState[] from, BbsState[] to, int x, int y) {
            ...
            for(int jj=0; jj<from.length; jj++) {
                if(block[jj]==1) {
                    if(from[jj].getDest() != null) {
                        for(int k=0; k<from[jj].getDest().size(); k++) {
                            int ii = from[jj].getDestAt(k);
                            if( ((block[ii]==0) && (from[ii].getNum()>=server[ii])) || (block[ii]==1) ) {
                                if( (to[jj].getNum()==(from[jj].getNum()-1)) && (to[ii].getNum()==(from[ii].getNum()+1)) ) {

                                        qMatrix.set(x, y, this.getDelta(from[jj].getNum())*this.serviceRate[jj]*this.calcF(from[jj].getNum())*this.routingMatrix.get(jj, ii)*this.isAccepted(from[jj].getNum(), ii));
                                        return true;
                                }
                            }
                        }
			...
			if(block[jj]==1) {
				if(from[jj].getDest() != null) {
					for(int k=0; k<from[jj].getDest().size(); k++) {
						...
						if( ((block[ii]==0) && (from[ii].getNum()<server[ii]))) {
							if( (to[jj].getNum()==from[jj].getNum()-1) && (to[ii].getNum()==from[ii].getNum()+1) ) {
								for(int x=0; x<(to[ii].getNsSize()); x++) {
									if( (to[ii].getNS(x) == (from[ii].getNS(x)+1)) ) {
										qMatrix.set(xx, yy, this.getDelta(from[jj].getNum())*this.serviceRate[jj]*this.calcF(from[jj].getNum())*this.routingMatrix.get(jj, ii)*this.routingMatrix.get(ii, from[ii].getDestAt(x)));
										return true;
									}
								}
							}
...
//										System.out.println("k: "+k+", "+from[j].toString());
//                                                        System.out.println("ns: "+ns+", serv: "+this.serviceRate[j]+", f: "+this.calcF(from[j].getNum())+", acc: "+this.isAccepted(from[i].getNum(), i));
                                                        qMatrix.set(xx, yy, ns*this.serviceRate[jj]*this.calcF(from[jj].getNum())*this.isAccepted(from[ii].getNum(), ii));
                                                        return true;
                                                }
                                        }
                                        else if( (to[jj].getNum()==from[jj].getNum()-1) && (to[ii].getNum()==from[ii].getNum()+1) ) {
...
//                                                    System.out.println("Entra |j="+j+" |i="+i+" |numTo="+to[j].getNum()+" |numFrom="+from[j].getNum());
                                                        if(ii==jj && (to[jj].getNum()==from[jj].getNum()) ) {
//                                                            System.out.println("Entra3 IF");
                                                                for(int x=0; x<(to[jj].getNsSize()); x++) {
                                                                        if( (to[jj].getNS(x) == (from[jj].getNS(x)+1)) ) {
                                                                                int ns = from[jj].getNSof(jj, ii);
//                                                                                System.out.println("BBS2 -> x: "+xx+" y: "+yy+", j: "+j+" to k: "+k);
//                                                                                System.out.println("FROM: "+this.printState(from));
//                                                                                System.out.println("TO: "+this.printState(to));
//                                                                                System.out.println("stampa(non 0):"+ns);
...
//                                                                                System.out.println(this.isAccepted(from[i].getNum(), i) );
                                                                                if(ns>0) {
                                                                                        qMatrix.set(xx, yy, ns*this.serviceRate[jj]*this.calcF(from[jj].getNum())*this.routingMatrix.get(jj, from[jj].getDestAt(x))*this.isAccepted(from[ii].getNum(), ii));
                                                                                        return true;
                                                                                }
                                                                        }
                                                                }
...
//											System.out.println("TO: "+this.printState(to));
											if(ns>0) {
												qMatrix.set(xx, yy, ns*this.serviceRate[jj]*this.calcF(from[jj].getNum())*this.routingMatrix.get(jj, from[jj].getDestAt(x)));
												return true;
											}
										}
									}
